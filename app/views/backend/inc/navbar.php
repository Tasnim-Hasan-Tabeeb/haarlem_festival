<nav class="navbar navbar-expand-lg bg-white border-bottom shadow-sm sticky-top" data-bs-theme="light">
    <div class="container-fluid">
        <a class="navbar-brand fw-bold text-dark" href="/home/dashboard">
            Admin Dashboard
        </a>

        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav"
                aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
        </button>

        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ms-auto align-items-lg-center gap-2">

                <li class="nav-item">
                    <div class="d-flex align-items-center gap-2 bg-light rounded-pill px-2 py-1 border">
                        <?php
                        $profile = $_SESSION['profile_picture'] ?? '';
                        ?>

                        <?php if (empty($profile)) : ?>
                            <img src="<?= htmlspecialchars($profile, ENT_QUOTES, 'UTF-8') ?>"
                                 class="rounded-circle user-avatar user-avatar-icon" alt="Profile Picture">
                        <?php else : ?>
                            <div class="d-flex align-items-center justify-content-center rounded-circle bg-secondary text-white"
                                 style="width:34px; height:34px;">
                                <i class="fas fa-user"></i>
                            </div>
                        <?php endif; ?>

                        <span class="fw-semibold text-dark small">
                            <?= htmlspecialchars($_SESSION['username'] ?? 'User', ENT_QUOTES, 'UTF-8'); ?>
                        </span>
                    </div>
                </li>

                <li class="nav-item">
                    <a class="btn btn-outline-dark btn-sm d-flex align-items-center gap-2" href="/login/logout">
                        <i class="fas fa-sign-out-alt"></i>
                        Logout
                    </a>
                </li>

            </ul>
        </div>
    </div>
</nav>