...
class HistoryRepository extends Repository
{
    ...
    {
        try {
            $stmt = $this->connection->prepare("SELECT ht.date, ht.start_time, ht.end_time, tl.name as language_name, '' as flag_image, htour.available_guides, htour.tour_id
    ...
    {
        try {
            $sql = "SELECT ht.date, ht.start_time, ht.end_time, tl.name as language_name, '' as flag_image, htour.available_guides, htour.tour_id
                FROM history_timeslots ht
                JOIN history_tours htour ON htour.timetable_id = ht.timetable_id
                JOIN tour_language tl ON tl.language_id = htour.language_id
    ...
    public function getOrderedTours(){
        try {
            $stmt = $this->connection->prepare("SELECT ht.date, ht.start_time, ht.end_time, tl.name as language_name, '' as flag_image, htour.available_guides
                                            FROM history_timeslots ht
                                            JOIN history_tours htour ON htour.timetable_id = ht.timetable_id
                                            JOIN tour_language tl ON tl.language_id = htour.language_id
